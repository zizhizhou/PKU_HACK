    ...
    if (!gameOver)
    {
        ...
        if (!pause)
        {
                ...
                (snake.position.x < 0) || (snake.position.y < 0))
            {
                ...
                gameOver = true;
                resource.PlaySoundInstant(resource.endSound);//游戏结束触发结束音效
            }
            //Rain movement
            if ((framesCounter%15) == 0)
            {
            ...
            for(int i  = 0; i < RAIN_NUM; i++)
            {
                if (CheckCollisionCircles(rain[i].position, sqrt(rain[i].size),snake.position,snake.radius))
                {
                    ...
                    if(rain[i].speed.y > 0)
                    {
                        snake.radius *= (0.9*alpha);
                    }
                    else if(rain[i].speed.y < 0)
                    {
                        snake.radius *= (1 - (1 - alpha)*0.9);
                    }
                    else
                    {
                        snake.radius *= alpha;
            ...
            for (int i = 0; i < RAIN_NUM;i++)
            {
                ...
                {
                    rain[i].size = SQUARE_SIZE * 3 * double(GetRandomValue(0,10))/double(10) ;
        ...
        if (snake.radius < 5){
            gameOver = true;
            resource.PlaySoundInstant(resource.endSound);//触发结束音效
        }
        framesCounter++;

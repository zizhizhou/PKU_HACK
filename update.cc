...
void Game::UpdateGame(void)
{
    if (!gameOver)
    {
        ...
        if (!pause)
        {
            ...
            //Collision
            if ((snake[0].position.x < (fruit.position.x + fruit.radius) && 
                 (snake[0].position.x + snake[0].radius) > fruit.position.x) &&
                 (snake[0].position.y < (fruit.position.y + fruit.radius) && 
                 (snake[0].position.y + snake[0].radius) > fruit.position.y))
            {
                snake[0].radius=sqrt(pow(snake[0].radius,2)+pow(fruit.radius,2));
